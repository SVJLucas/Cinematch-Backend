...
async def post_rating(rating: RatingPost, db: Reference = Depends(get_database)):
    ...
    user_id = rating_data['user_id']
    movie_id = rating_data['movie_id']
    user_ratings = management.get_by_field(field='user_id', value=user_id, db=db)
...
async def delete_rating(rating_id: str, db: Reference = Depends(get_database)) -> RatingResponse:
    """
...
async def put_rating(rating_id: str, rating: RatingUpdate, db: Reference = Depends(get_database)) -> RatingResponse:
    """
